...
impl FeatureMsrAdjustment {
    ...
    /// and the ones that should be denied based on `self` and the given
    /// `feature_msrs`.
    #[cfg(feature = "kvm")]
    pub(in crate::x86_64) fn adjust_to(
        adjustments: &[(RegisterAddress, FeatureMsrAdjustment)],
        feature_msrs: &[MsrEntry],
    ) -> Result<FeatureMsrUpdate, MissingMsrEntriesError> {
        let mut output_feature_msrs = Vec::with_capacity(feature_msrs.len());
        let mut denied_feature_msrs = Vec::new();

        let mut not_found = false;
        for MsrEntry { index, data } in feature_msrs.iter().copied() {
            if let Some((_, adjustment)) =
                adjustments.iter().find(|(reg_addr, _)| reg_addr.0 == index)
            {
                let adjusted = (data & adjustment.mask) | adjustment.replacements;
                output_feature_msrs.push(MsrEntry {
                    index,
                    data: adjusted,
                });
            } else {
                denied_feature_msrs.push(RegisterAddress(index));
            }
        }
        ...
        if !not_found {
            Ok(FeatureMsrUpdate {
                msr_based_features: output_feature_msrs,
                denied_msr_based_features: denied_feature_msrs,
            })
        } else {
            Err(MissingMsrEntriesError)
        }
    }
}

pub struct FeatureMsrUpdate {
    pub msr_based_features: Vec<MsrEntry>,
    pub denied_msr_based_features: Vec<RegisterAddress>,
}
