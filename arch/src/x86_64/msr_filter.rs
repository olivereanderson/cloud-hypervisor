...
use hypervisor::MsrFilterRange;

use super::Error;

/// The maximum number of MSR filter ranges an MSR filter may consist of.
const MAX_FILTERS: usize = {
    #[cfg(feature = "kvm")]
    {
        hypervisor::kvm::KVM_MSR_FILTER_MAX_RANGES
    }
    #[cfg(not(feature = "kvm"))]
    {
        // TODO: Change this when adding support for CPU profiles with MSHV
        16
    }
};

/// THE maximum number of bytes the bitmap arena used for the filter may occupy.
/// This is to ensure that we do not allocate too much memory for the bitmaps in
/// the filter ranges.
pub const MAX_BITMAP_SIZE: usize = MAX_FILTERS * 1024 * 1024;

/// Apply a filter which denies guests any kind of access to the MSRs in `denied_msrs`.
///
/// # Assumptions
///
/// This function may explicitly mark certain MSRs different from those in `denied_msrs` as
/// both READ + Write permitted. We assume that the hypervisor will permit this filter being set
/// regardless and rather injects an exception if guests attempt to read/modify these MSRs in anyway
/// that is incompatible with the hardware and/or hypervisor.
///
/// # Errors
///
/// This errors if any of the following conditions hold:
///
/// 1. Too much memory is required to construct the MSR filter that covers all of the denied MSRs.
/// 2. The VM/Hypervisor fails to apply the MSR filter.
pub fn filter_denied_msrs(
    mut denied_msrs: Vec<u32>,
    vm: &dyn hypervisor::Vm,
) -> Result<(), crate::Error> {
    if denied_msrs.is_empty() {
        return Ok(());
    }
    denied_msrs.sort_unstable();

    for msr in &denied_msrs {
        log::debug!("MSR:={msr:#x} is set to be denied");
    }

    let mut bitmap_arena = Vec::new();
    let (filter, num_filter_ranges) = denied_to_filter(&denied_msrs, &mut bitmap_arena)?;

    if let Err(e) = vm.msr_filter(&filter[..num_filter_ranges], false) {
        // Log more details at the debug level. Note that this error is likely to be reproducible and happens close to startup, hence
        // it should be relatively easy to set the necessary log level if/when debugging becomes desirable.
        for filter_range in &filter[..num_filter_ranges] {
            // We want to encode the bitmap as a string of the form "[<byte 0 hex encoded>, <byte 1 hex encoded> ,... <last byte hex encoded>]"
            let mut bitmap_hex_encoded = String::with_capacity((4 * filter_range.bitmap.len()) + 2);
            let _ = write!(&mut bitmap_hex_encoded, "[");
            for b in filter_range.bitmap.iter() {
                let _ = write!(&mut bitmap_hex_encoded, "{b:#x},");
            }
            // Remove the final "," from the string
            bitmap_hex_encoded.pop();
            let _ = write!(&mut bitmap_hex_encoded, "]");
            log::debug!(
                "Failed to set MSR filter containing filter range: base:={:#x}, nmsrs:={:#x}, bitmap:={}",
                filter_range.base,
                filter_range.nmsrs,
                bitmap_hex_encoded
            );
        }
        Err(Into::into(Error::MsrFilter(e)))
    } else {
        Ok(())
    }
}
...
fn denied_to_range_indices<'a>(
    ...
    let mut eval_deny_range = |r_s: usize, r_e: usize| {
        const LAST_IDX: usize = MAX_FILTERS - 1;
...
fn range_indices_to_filter<'a>(
    denied_sorted: &[u32],
    range_indices: &[(usize, usize)],
    bitmap_arena: &'a mut Vec<u8>,
) -> Result<[MsrFilterRange<'a>; MAX_FILTERS], Error> {
    let mut out = [MsrFilterRange::default().with_read_write_flags(); MAX_FILTERS];
    let bytes_to_allocate: usize = range_indices
        .iter()
        .copied()
        .map(|(s, e)| ((denied_sorted[e] - denied_sorted[s]) + 1).div_ceil(8))
        .map(|v| v as usize)
        .sum();

    if bytes_to_allocate > MAX_BITMAP_SIZE {
        return Err(Error::MsrFilterTooLarge(bytes_to_allocate));
    }
...
/// that collectively deny each of the MSRs specified in `denied_sorted`.
///
...
/// This function can only error if more than [`MAX_BITMAP_SIZE`] bytes are required
/// to construct the filters.
fn denied_to_filter<'a>(
    denied_sorted: &[u32],
    bitmap_arena: &'a mut Vec<u8>,
) -> Result<([MsrFilterRange<'a>; MAX_FILTERS], usize), Error> {
    let mut range_indices_buffer = [(0, 0); MAX_FILTERS];
    let range_indices = denied_to_range_indices(denied_sorted, &mut range_indices_buffer);

    range_indices_to_filter(denied_sorted, range_indices, bitmap_arena)
        .map(|filter| (filter, range_indices.len()))
}
...
mod unit_tests {
    ...
    use super::{MAX_BITMAP_SIZE, MAX_FILTERS, denied_to_filter};

    /// transforms entries out of the x2apic MSR range and sorts + dedups the vector
    ...
    proptest! {
        #[test]
        fn denied_to_filer_works_short(prepared_msrs in (prop::collection::vec(0..u32::MAX, 1..MAX_FILTERS)).prop_map(prepare)) {
            let mut bitmap_arena = Vec::new();
            let Ok((filter, num_filter_ranges)) = denied_to_filter(&prepared_msrs, &mut bitmap_arena) else {
                return Ok(());
            };
            let mut recomputed_msrs = filter_to_msrs(&filter[..num_filter_ranges]);
    ...
    proptest! {
        #[test]
        fn denied_to_filer_works(prepared_msrs in (prop::collection::vec(0..u32::MAX, 17..70)).prop_map(prepare)) {
            let mut bitmap_arena = Vec::new();
            let Ok((filter, num_filter_ranges)) = denied_to_filter(&prepared_msrs, &mut bitmap_arena) else {
                return Ok(());
            };
    ...
    fn catches_attempt_to_allocate_too_much_memory() {
        ...
        let denied_msrs: Vec<u32> = (0..MAX_FILTERS * 8 * 2)
            ...
            .collect();
        let _ = denied_to_filter(&denied_msrs, &mut bitmap_arena).unwrap_err();
    }
}
