...
// build.rs generates a CpuProfiles enum with a variant for each
// CPU profile in arch/x86_64/cpu_profiles and also has the default
// host variant as well.
//
// Furthermore the build script also generates the functions
// `compressed_cpuid_data`, `compressed_msr_data` for obtaining the
// compressed JSON data associated with the given cpu profile.
include!(concat!(env!("OUT_DIR"), "/generated_cpu_profiles.rs"));
...
impl CpuProfile {
    ...
    fn cpuid_data(&self) -> Option<CpuidProfileData> {
        const ESTIMATED_CPUID_CPU_PROFILE_DATA_COMPRESSION_RATIO: usize = 32;

        // The compressed_cpuid_data function is generated by build.rs
        let compressed: &[u8] = compressed_cpuid_data(self)?;
        let data: CpuidProfileData = {
            serde_json::from_slice(&Self::decompress_cpu_profile_data(
                compressed,
                ESTIMATED_CPUID_CPU_PROFILE_DATA_COMPRESSION_RATIO,
            ))
            .expect("Should be able to deserialize CPU profile CPUID data")
        };
    ...
    fn msr_data(&self) -> Option<MsrProfileData> {
        const ESTIMATED_MSR_CPU_PROFILE_DATA_COMPRESSION_RATIO: usize = 4;

        // compressed_msr_data is created by build.rs
        let compressed: &[u8] = compressed_msr_data(self)?;
        serde_json::from_slice(&Self::decompress_cpu_profile_data(
            compressed,
            ESTIMATED_MSR_CPU_PROFILE_DATA_COMPRESSION_RATIO,
        ))
        .expect("Should be able to deserialize CPU profile MSR data")
    }
    ...
    /// Decompress the `compressed` byte slice.
    ///
    /// The `estimated_compression_ratio` is just used for optimizing the number of necessary allocations
    /// and does not have to be accurate.
    fn decompress_cpu_profile_data(
        compressed: &[u8],
        estimated_compression_ratip: usize,
    ) -> Vec<u8> {
        let mut decoder = ZlibDecoder::new(compressed);
        // Don't expect more than a 32x compression ratio
        let mut v = Vec::with_capacity(compressed.len() * estimated_compression_ratip);
        decoder
            .read_to_end(&mut v)
            .expect("Should be able to decompress CPU profile data");
        v
    }
}
