...
fn required_msr_updates(
    ...
    if !required.is_subset(&queryable_msrs) {
        ...
        for missing_msr in required.difference(&queryable_msrs) {
            ...
            missing_mandatory_msrs = true;
            error!("Did not find MSR {missing_msr:#x}, but it is required by the CPU profile");
        }

        if missing_mandatory_msrs {
            return Err(Error::CpuProfileMissingMsr);
        }
    }

    // Our MSR state buffer should consist of the MSR indices that are contained in the set of required MSRs and the feature MSRs to be set.
    ...
    let mut msr_state_buffer: Vec<MsrEntry> = msr_index_list
        .iter()
        .copied()
        .filter(|msr| required.contains(msr))
        .map(|index| MsrEntry { index, data: 0 })
        .chain(adjusted_feature_msrs.iter().map(|entry| MsrEntry {
            index: entry.index,
            data: 0,
        }))
        .collect();
    // Remove any duplicate entries
    msr_state_buffer.sort_by_key(|entry| entry.index);
    msr_state_buffer.dedup();
...
mod unit_tests {
    ...
    fn failure_on_missing_msrs(
        ...
        if host_feature_msrs.iter().any(|entry| entry.index == removed_msr) {
            prop_assert!(required_msr_updates(profile_data, &host_feature_msrs, &host_msr_index_list, true).is_ok());
        } else {
            prop_assert!(required_msr_updates(profile_data, &host_feature_msrs, &host_msr_index_list, true).is_err());
            }
        }
    }
    ...
    fn required_msr_updates_expected_adjustments() {
        ...
        // Assert that the MSR state buffer satisfies our expectations as well.
        required_updates
            .msr_state_buffer
        ...
        assert_eq!(
            ...
            &required_updates.msr_state_buffer
        );
    }
}
