...
pub(in crate::x86_64) const HYPERV_MSRS: [u32; 64] = [
...
#[cfg(all(test, feature = "kvm", feature = "cpu_profile_generation"))]
mod tests {
    use super::*;
    use crate::x86_64::msr_definitions::intel::{
        INTEL_MSR_FEATURE_DEFINITIONS, PERMITTED_IA32_MSRS,
    };
    use crate::x86_64::msr_definitions::kvm::PROFILE_PERMITTED_KVM_MSRS;

    // If this can be assumed than that simplifies some things.
    //
    // NOTE: It is perfectly possible to make this a compile time check instead,
    // but that is more cumbersome hence we leave that for later.
    #[test]
    fn does_not_intersect_other_permitted_msr_sets() {
        for msr in HYPERV_MSRS {
            assert!(
                !INTEL_MSR_FEATURE_DEFINITIONS
                    ...
                    .iter()
                    .map(|r| r.0.0)
                    .chain(PERMITTED_IA32_MSRS)
                    .chain(PROFILE_PERMITTED_KVM_MSRS)
                    .any(|other_permitted_msr| other_permitted_msr == msr)
            );
        }
    }
}
