...
/// Generates the feature MSRs that need to be set in order to be compatible with the given CPU profile (if any).
///
/// An error is returned if any feature MSR required by the given CPU profile is not reported by the hypervisor.
///
...
pub fn generate_required_feature_msr_updates(
    hypervisor: &dyn hypervisor::Hypervisor,
    cpu_profile: CpuProfile,
) -> super::Result<Option<Vec<MsrEntry>>> {
    ...
    let adjusted_feature_msrs = cpu_profile.required_feature_msr_updates(&host_feature_msrs)?;

    if let Some(required_updates) = &adjusted_feature_msrs {
